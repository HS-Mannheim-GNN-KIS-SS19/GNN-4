...
public class Network {
	...
	public void activate(double inVector[])
	{
		...
		for (int i=0;i<numOfWeights;i++){
			if (i<numOfInputs){
				sum += neuron[numOfNeurons-1].weight[i] * inVector[i];
			}
			if (i>=numOfInputs ){
				sum += neuron[numOfNeurons-1].weight[i] * neuron[i-numOfInputs].output;
			}	
		}
		neuron[numOfNeurons-1].output = ThreshFunction(sum);
